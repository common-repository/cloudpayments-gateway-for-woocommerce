...
{
  public $id = 'wc_cloudpayments_gateway';
  public $enabled;
  public string $enabledDMS;
  public string $kassa_taxtype;
  public string $enabledOPC;
  public string $DMS_AU_status;
  public string $status_cancel;
  public string $status_pay;
  public string $public_id;
  public string $api_pass;
  public string $currency;
  public string $kassa_enabled;
  public string $delivery_taxtype;
  public string $kassa_taxsystem;
  public string $groupSeparator;
  public string $kassa_skubarcode;
  public string $kassa_method;
  public string $kassa_object;
  ...
  public function __construct()
    {
        ...
        $this->enabledOPC         = $this->get_option('enabledOPC');
        $this->DMS_AU_status      = $this->get_option('DMS_AU_status');
        $this->status_cancel     = $this->get_option('status_cancel');
        ...
        $this->groupSeparator     = $this->get_option('groupSeparator');
        ...
        /** API init */
        $api = new CloudPayments_Api($this->enabledDMS, $this->status_cancel, $this->status_pay, $this->api_pass, $this->DMS_AU_status, $this->public_id);
    ...
    public function process_payment($order_id)
    {
        ...
        if (empty($_POST['cp_card']) or $_POST['cp_card'] == 'widget') {
          if (isset($_POST['cp_save_card'])) {
            ...
            return array(
                'result'   => 'success',
                'redirect' => $this->enabledOPC == 'yes'
                    ? $this->get_return_url($order) . "&" . http_build_query(['cp_save_card' => $cp_save_card])
                    : home_url('/wc-api/cloud_payments_widget?') . http_build_query($query)
            );
        }
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'enabledOPC'       => array(
                'title'   => __('Вызывать оплату на странице сформированного заказа', 'woocommerce'),
                'type'    => 'checkbox',
                'label'   => __('Вызывать оплату на странице сформированного заказа', 'woocommerce'),
                'default' => 'no'
            ),
            'order_text'       => array(
            ...
            'groupSeparator' => array(
                'title'       => __('Разделитель GS', 'woocommerce'),
                'type'        => 'text',
                'description' => 'Укажите Разделитель GS, если при отгрузке передаете маркировку в ЧЗ',
                'default'     => '',
                'desc_tip'    => false,
            ),
            'kassa_method'     => array(
    ...
    public function update_order_status($order_id, $old_status, $new_status)
    {
        if ($this->kassa_enabled == 'yes') {
            ...
            if ("wc-" . $new_status == $this->status_cancel && $order->get_payment_method() === 'wc_tiptoppay_gateway') {
              $auth  = base64_encode($this->public_id . ":" . $this->api_pass);
              
              $response = wp_remote_post('https://api.cloudpayments.ru/payments/find', array(
                'timeout'     => 30,
                'redirection' => 5,
                'httpversion' => '1.0',
                'blocking'    => true,
              ...
              if (!empty($response_body) and $response_body['Success']) {
                wp_remote_post('https://api.cloudpayments.ru/payments/refund', array(
                  'timeout'     => 30,
                  'redirection' => 5,
                  'httpversion' => '1.0',
                  'blocking'    => true,
                  'headers'     => array('Authorization' => 'Basic ' . $auth, 'Content-Type' => 'application/json', 'X-Request-ID' => md5(date("d-m-Y H:i:s"))),
                  'body'        => json_encode(array(
                    "TransactionId" => $response_body['Model']['TransactionId'],
                    "Amount" => $response_body['Model']['Amount'],
                    "JsonData" =>  "string"
                  )),
                  'cookies'     => array()
                ));
    ...
    public function send_receipt($order, $type, $old_status, $new_status)
    {
        ...
        foreach ($cart as $item_data) {
            ...
            $method = "wc-" . $new_status == $this->status_delivered ? 4 : (int)$this->kassa_method;
            $rawMarkCodes = array_filter(array_map(fn ($meta_data) => $meta_data->get_data(), $item_data->get_meta_data()), fn ($data) => $data['key'] == 'rawMarkCode');
            if (count($rawMarkCodes)) {
                foreach ($rawMarkCodes as $rawMarkCode) {
                    $items[] = apply_filters(
                        ...
                        array(
                            ...
                            'quantity' => 1,
                            'amount'   => number_format( floatval( $item_data->get_total() ) / $item_data->get_quantity(), 2, '.', '' ),
                            'vat'      => $this->kassa_taxtype == 'null' ? null : $this->kassa_taxtype,
                            'rawMarkCode'    => $rawMarkCode['value'],
            ...
            $total_amount += number_format(floatval($item_data->get_total()), 2, ".", '');
        }
        ...
        if ($order->get_total_shipping()) {
            ...
            $total_amount += number_format(floatval($order->get_total_shipping()), 2, ".", '');
        }
